...
impl<'a> Workbook<'a> {
	pub fn new(directory: &str, author: &'a str) -> Workbook<'a> {
		...
		if let Ok(mut xml) = xml_writer_for_file(&helper) {
			...
			xml.begin_elem("Relationships");
				xml.attr_esc("xmlns", "http://schemas.openxmlformats.org/package/2006/relationships");

				xml.begin_elem("Relationship");
				xml.attr_esc("Id", "styles");
				xml.attr_esc("Target", "styles.xml");
				xml.attr_esc("Type", "http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles");
				xml.end_elem();

				xml.begin_elem("Relationship");
				xml.attr_esc("Id", "sharedstrings");
				xml.attr_esc("Target", "sharedStrings.xml");
				xml.attr_esc("Type", "http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings");
				xml.end_elem();
			...
			if let Ok(mut content) = xml_writer_for_file(&helper) {
				...
				content.begin_elem("Types");
					content.attr_esc("xmlns", "http://schemas.openxmlformats.org/package/2006/content-types");
					...
					content.begin_elem("Default");
						content.attr_esc("ContentType", "application/vnd.openxmlformats-package.relationships+xml");
					...
					content.begin_elem("Override");
						content.attr_esc("ContentType", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml");
						content.attr_esc("PartName", "/xl/workbook.xml");
					content.end_elem();

					content.begin_elem("Override");
						content.attr_esc("ContentType", "application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml");
						content.attr_esc("PartName", "/xl/styles.xml");
					content.end_elem();

					content.begin_elem("Override");
						content.attr_esc("ContentType", "application/vnd.openxmlformats-officedocument.spreadsheetml.sharedStrings+xml");
						content.attr_esc("PartName", "/xl/sharedStrings.xml");
					content.end_elem();

					content.begin_elem("Override");
						content.attr_esc("ContentType", "application/vnd.openxmlformats-package.core-properties+xml");
				...
				if let Ok(mut workbook) = xml_writer_for_file(&helper) {
					...
					workbook.begin_elem("workbook");
						workbook.attr_esc("xmlns", "http://schemas.openxmlformats.org/spreadsheetml/2006/main");
						workbook.attr_esc("xmlns:r", "http://schemas.openxmlformats.org/officeDocument/2006/relationships");
	...
	fn write_core(&self) {
		...
		if let Ok(mut xml) = xml_writer_for_file(&helper) {
			...
			xml.begin_elem("cp:coreProperties");
				xml.attr_esc("xmlns:cp", "http://schemas.openxmlformats.org/package/2006/metadata/core-properties");
	...
	pub fn new_worksheet(&mut self, title: &str, headerRows: u32) -> Worksheet<'a> {
		self.sheetCount += 1;
		let idx = self.sheetCount;

		self.worksheets.begin_elem("Relationship");
			self.worksheets.attr_esc("Id", &format!("sheet{}", idx));
			self.worksheets.attr_esc("Target", &format!("worksheets/sheet{}.xml", idx));
			self.worksheets.attr_esc("Type", "http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet");
		...
		self.content.begin_elem("Override");
			self.content.attr_esc("ContentType", "application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml");
			self.content.attr_esc("PartName", &format!("/xl/worksheets/sheet{}.xml", idx));
	...
	pub fn xlsx(&mut self, outfile: &str) {
		...
		zip.start_file("_rels/", zip::CompressionMethod::Stored);
		zip.start_file("docProps/", zip::CompressionMethod::Stored);
		zip.start_file("xl/", zip::CompressionMethod::Stored);
		zip.start_file("xl/_rels/", zip::CompressionMethod::Stored);
		zip.start_file("xl/worksheets/", zip::CompressionMethod::Stored);
	...
	fn zip_file(&mut self, file: &str, zip: &mut zip::ZipWriter<File>) {
		{
			zip.start_file(file, zip::CompressionMethod::Stored);
		}
