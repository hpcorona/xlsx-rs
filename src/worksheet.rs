...
impl<'a> Worksheet<'a> {
    pub fn new(path: &PathBuf, idx: u32, header_rows: u32) -> Worksheet<'a> {
        let mut file_path = path.clone();
        file_path.push("xl");
        file_path.push("worksheets");
        file_path.push(format!("sheet{}.xml", idx));

        if let Ok(mut xml) = xml_writer_for_file(&file_path) {
            xml.dtd("UTF-8");
